...
class TD3Agent:
    ...
    def update(self, batch_size):
        state_batch, action_batch, reward_batch, next_state_batch, masks = self.replay_buffer.sample(batch_size)    # 随机采样，再把采样的经验都 append 成数组
        state_batch = torch.FloatTensor(state_batch).to(self.device)    # 都转成浮点型 tensor 张量
        action_batch = torch.FloatTensor(action_batch).to(self.device)
        reward_batch = torch.FloatTensor(reward_batch).to(self.device) # reward不用reshape，因为存的时候就是以[reward]的形式存的
        next_state_batch = torch.FloatTensor(next_state_batch).to(self.device)
        masks = torch.FloatTensor(masks).reshape((batch_size,1)).to(self.device) # masks需要reshape   # 是否done
        ...
        # tt = expected_Q.detach()
        critic1_loss = F.mse_loss(curr_Q1, expected_Q.detach())     # 采样的128个（比如是128），共同供给Critic学习
        critic2_loss = F.mse_loss(curr_Q2, expected_Q.detach())
        
        # update critics
        self.critic1_optimizer.zero_grad()  # 梯度初始化为0
        critic1_loss.backward()             # 计算梯度
        self.critic1_optimizer.step()       # 更新梯度
        ...
        if(self.update_step % self.delay_step == 0):
            ...
            qq = self.critic1(state_batch, self.actor(state_batch))
